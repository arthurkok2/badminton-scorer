---
title: Cloud Session History and Stats Design
author: arthur.kok
date: 2026-05-18
status: implemented
tags: [scoring, engine]
...
- Global player search and creation are only attempted for named signed-in users. Session setup debounces player search input before issuing the cloud lookup, so quick typing does not send one Firestore query per keystroke. If stale session setup UI is reachable while signed out or anonymous, player search clears local results and does not issue a Firestore read that would be rejected by rules. Recoverable cloud failures surface through an app-level toast viewport so the user sees the problem without blocking the current session.
