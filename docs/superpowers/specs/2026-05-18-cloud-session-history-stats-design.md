...
- Global player search and creation are only attempted for named signed-in users. If stale session setup UI is reachable while signed out or anonymous, player search clears local results and does not issue a Firestore read that would be rejected by rules.
