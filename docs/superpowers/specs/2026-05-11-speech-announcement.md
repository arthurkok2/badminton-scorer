...
```
Example: `"Team Ay, Alice serving, 3-1."`

**Winner:**
```
Game. {team} wins, {winning score}-{losing score}.
```
Example: `"Game. Team Ay wins, 21-10."`
...
An earlier approach inserted a comma: `"Team, A"`. The pause separated the words but most TTS engines still pronounced the isolated "A" as a schwa ("uh"). Replacing with "Ay" is more reliable because TTS engines respond to phonetic spelling directly.

## Why the format order matters

The original format was `"{team} serving, {player}, {score}."`. TTS reads "Team Ay serving" as a phrase before pausing for the player name, which sounds unnatural. The updated order `"{team}, {player} serving, {score}."` announces who first (team + player) then what (serving) then score, matching how a human umpire would call it.

## Testing

- Serving announcement matches `"{team}, {player} serving, {score}."` format.
- Winner announcement matches `"Game. {team} wins, {score}."` format.
- Team A → "Team Ay", Team B → "Team Bee" in both contexts.
